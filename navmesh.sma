...
new Float:g_fNorthEast, g_fSouthWest;
...
public LoadNavigationMap()
{
	...
	ArrayPushCell(g_aAreaID, iID);

	server_print("m_id: %d", iID);

	// update nextID to avoid collisions
	ArrayPushCell(g_aAreaNextID, iID + 1);

	server_print("m_nextID: %d", iID + 1);

	// load attribute flags
	fread(iFile, iFlags, BLOCK_CHAR);
	ArrayPushCell(g_aAttributeFlags, iFlags);
	...
	for(i = 0; i < iAreas; i++)
	{
		...
		server_print("iArea: %d id: %d", i, iID);
